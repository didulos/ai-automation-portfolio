...
def parse_memos_from_blocks(blocks: list) -> list:
    ...
    두 가지 패턴을 처리:
    1. paragraph 블록에 "번호. 내용" 형태로 작성된 경우
    2. numbered_list_item 블록 (Notion이 번호를 자동 매김, 텍스트에 번호 없음)
    ...
    auto_number = 0  # numbered_list_item용 자동 번호
    ...
    for block in blocks:
        ...
        # 방법 1: 텍스트 안에 "번호. 내용" 패턴이 있는 경우
        match = number_pattern.match(text)
        if match:
            number = int(match.group(1))
            content = match.group(2).strip()
            auto_number = number  # 자동 번호 동기화
            memos.append({
                "number": number,
                "text": content,
                "block_ids": [block_id],
        ...
        # 방법 2: numbered_list_item인데 번호가 텍스트에 없는 경우
        ...
        # 방법 3: 기타 블록 (paragraph, bulleted 등) — 독립 메모로 취급
        elif block_type in ("paragraph", "bulleted_list_item", "to_do",
                            "toggle", "callout", "quote",
                            "heading_1", "heading_2", "heading_3"):
            # 내용이 있는 블록은 독립 메모로 처리
            if len(text) > 2:  # 너무 짧은 건 무시 (구분선 등)
                auto_number += 1
                memos.append({
                    "number": auto_number,
                    "text": text,
